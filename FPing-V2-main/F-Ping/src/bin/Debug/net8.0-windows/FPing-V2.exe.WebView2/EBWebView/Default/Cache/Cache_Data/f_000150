document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- API Configuration ---
    const apiKey = '6eb03bd3-7544bcce-13f935eb-5e13821e'; // Your key
    ...
    function createItemCard(item) {
        ...
        // Add rarity class for potential specific styling (e.g., borders)
        if (item.rarity?.id) {
            itemCard.classList.add(`rarity-${item.rarity.id.toLowerCase()}`);
        }

        const { color1, color2, color3 } = item.rarity?.colors || {};
        if (color1 && color2) {
            itemCard.style.backgroundImage = `linear-gradient(to bottom, #${color1}, #${color2}${color3 ? ', #' + color3 : ''})`;
        } else {
            itemCard.style.backgroundColor = '#2c2c2c'; // Default
        }
